CREATE TABLE WorkHoursLogs (
	...
	CREATE TRIGGER tr_WorkHoursInsert ON WorkHours AFTER INSERT AS
		INSERT INTO WorkHoursLogs(LogDateTime, NewEmployeeId, NewDate, NewTask, NewHours, NewComment, Command)
		SELECT GETDATE(), i.EmployeeId, i.Date, i.Task, i.Hours, i.Comment, 'INSERT'
		FROM inserted i

	GO

	CREATE TRIGGER tr_WorkHoursUpdate ON WorkHours AFTER UPDATE AS
		INSERT INTO WorkHoursLogs(LogDateTime, OldEmployeeId, OldDate, OldTask, OldHours, OldComment, 
			NewEmployeeId, NewDate, NewTask, NewHours, NewComment, Command)
		SELECT GETDATE(), d.EmployeeId, d.Date, d.Task, d.Hours, d.Comment, 
			i.EmployeeId, i.Date, i.Task, i.Hours, i.Comment, 'UPDATE'
		FROM inserted i, deleted d

	GO

	CREATE TRIGGER tr_WorkHoursDelete ON WorkHours AFTER DELETE AS
		INSERT INTO WorkHoursLogs(LogDateTime, OldEmployeeId, OldDate, OldTask, OldHours, OldComment, Command)
		SELECT GETDATE(), d.EmployeeId, d.Date, d.Task, d.Hours, d.Comment, 'DELETE'
		FROM deleted d
